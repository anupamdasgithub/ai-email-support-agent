...
customer email triggers a process that fetches prior conversation context,
queries a RAG knowledge base, lets an agent decide, and either resolves the case
autonomously or escalates to a human — with the resolution saved back to
long-term memory. An "Agent as a Judge" step evaluates each interaction.
...
Deploy `process/bpmn/ai-email-support-agent.bpmn` to the running cluster via
Web Modeler.
...
- **PII redaction** - the `PiiRedactor` in `workers` deterministically redacts
  ...
  (`PiiRedactorTest`).
- **Out-of-scope rejection** - the agent handles only loan-domain requests;
  off-domain requests are refused and escalated to a human via the judge.
...
(`git tag` scheme `bpmn-v<N>-good`).

## Observability

- **Operate / Tasklist** - process/task state
